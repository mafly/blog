    ...
    <?php if($this->allow('comment')): ?>
        <div id="<?php $this->respondId(); ?>" class="respond">
            ...
            <form method="post" action="<?php $this->commentUrl() ?>" id="comment_form">
                ...
                <?php endif; ?>
                <label><div class="cancel-comment-reply"><?php $comments->cancelReply(); ?></div><?php _e('评论内容'); ?><textarea rows="3" cols="50" name="text"  placeholder="添加一条评论" id="comment" class="textarea"  onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('submit').click();return false};"><?php $this->remember('text'); ?></textarea></label>
				<input type="submit" value="<?php _e('确认提交 / Ctrl + Enter'); ?>" class="submit" id="submit"/>
            </form>
        </div>
    <?php else: ?>
        <p>评论已关闭</p>
